 ...
 *
 * Returns the platform-wide policy catalog joined with the current tenant's
 * per-policy state. Any member of the tenant can read. (Read-only for
 * non-admins — writes go through PATCH /api/policy-library/[code] which
 * requires canEditTenant.)
 */
...
export async function GET(req: NextRequest) {
  ...
  const stateByCode = new Map<string, unknown>(
    (states ?? []).map((s) => [(s as { policy_code: string }).policy_code, s]),
  );
