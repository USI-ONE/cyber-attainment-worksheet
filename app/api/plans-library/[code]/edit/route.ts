...
/**
 * POST /api/plans-library/[code]/edit
 *
 * Save inline text edits to the document attached to one plans-library
 * entry as a brand-new version. Mirrors the file-upload "new revision"
 * flow but with the edited body coming from the request JSON instead
 * of a multipart file.
 *
 * Request JSON:
...
const MAX_BYTES = 5 * 1024 * 1024; // 5 MB of plain text is already absurd
...
export async function POST(
  ...
  // Repoint tenant_plans → new doc; advance review dates.
