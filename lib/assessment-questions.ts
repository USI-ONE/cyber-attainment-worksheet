...
export const CONTROL_QUESTIONS: Record<string, ControlQuestionnaire> = {
  ...
  'GV.OV-02': {
    q1: 'Is there a coverage matrix or checklist mapping cybersecurity strategy elements to current risks, regulations, threats, and stakeholder expectations? Pull it.',
    q1_hint: 'Yes = explicit coverage matrix exists. Partial = informal cross-referencing in different documents. No = no coverage check.',
  ...
  'ID.AM-04': {
    ...
    q1_hint: 'Yes = comprehensive inventory with ≥4 fields populated. Partial = inventory exists but data thin. No = no inventory or only AP records.',
  ...
  'ID.AM-07': {
    ...
    q1_hint: 'Yes = comprehensive data map with all regulated types and system mapping. Partial = some data types covered, others missing. No = no data map.',
  ...
  'ID.RA-03': {
    ...
    q2: 'Are recorded threats tied to specific assets and data categories so a treatment plan can actually be designed?',
    q2_hint: 'Yes = threats are scoped to assets/data with specificity. Partial = some scoped, some not. No = threats lack scope.',
  ...
  'ID.IM-03': {
    ...
    q3: 'Are operational metrics reviewed at executive level (helpdesk volume, alert volume, MTTR)? Pull the most recent board pack or exec briefing.',
    q3_hint: 'Yes = exec-level visibility with discussion. Partial = metrics shown but not discussed. No = exec-level operational visibility absent.',
  ...
  'ID.IM-04': {
    ...
    q3: 'Are external coordination contacts (insurer, IR retainer, legal counsel, law enforcement, regulators) listed and verified within the past 12 months?',
    q3_hint: 'Yes = all named contacts verified within 12 months. Partial = some verified, some stale. No = contacts not verified.',
  ...
  'PR.AA-04': {
    ...
    q2: 'Are session cookies marked Secure, HttpOnly, and SameSite where applicable? Run a security scan on the login page (e.g., Mozilla Observatory, SSL Labs).',
    q2_hint: 'Yes = scan shows A grade or equivalent. Partial = some flags missing. No = poor scan grade or never scanned.',
  ...
  'PR.AT-02': {
    ...
    q3: 'Are role-specific competencies validated via certification, lab exercise, or capability assessment (not just course completion)? Show evidence.',
    q3_hint: 'Yes = capability validation in addition to completion. Partial = completion-only with informal verification. No = completion is the only measure.',
  ...
  'PR.DS-10': {
    q1: 'Are DLP rules deployed covering copy/paste, screen capture, and clipboard for sensitive data? Show the policy in M365 / SaaS-DLP / endpoint-DLP.',
    q1_hint: 'Yes = DLP rules cover ≥3 data-in-use vectors. Partial = some vectors covered. No = no DLP for data-in-use.',
    ...
    q3: 'Are AI tool data interactions covered by DLP (browser extensions, ChatGPT-equivalents, copy/paste into AI consoles)?',
    q3_hint: 'Yes = AI interactions monitored or blocked per data classification. Partial = some controls. No = AI not addressed.',
  ...
  'PR.IR-04': {
    ...
    q1_hint: 'Yes = comprehensive capacity dashboard. Partial = some metrics tracked. No = no capacity view.',
  ...
  'DE.CM-02': {
    ...
    q3: 'Are physical-monitoring records reviewed for trends and fed into insider-risk or facility-security reviews?',
    q3_hint: 'Yes = review with documented downstream use. Partial = review happens but findings disconnected. No = no review.',
    q4_prompt: 'Describe a physical-monitoring improvement or finding in the past year.',
  },
  'DE.CM-03': {
    q1: 'What user-activity logs are captured (sign-in, privileged actions, data access, file movement)? Where are they stored?',
    q1_hint: 'Yes = comprehensive user-activity logging across systems. Partial = some logging. No = limited or no user logging.',
    ...
    q3: 'Are monitoring scope and proportionality disclosed in the AUP (Acceptable Use Policy)? Has legal reviewed the scope?',
    q3_hint: 'Yes = monitoring disclosed AND legal-reviewed. Partial = disclosed but not legal-reviewed. No = monitoring undisclosed.',
  ...
  'DE.CM-06': {
    ...
    q1_hint: 'Yes = comprehensive vendor-activity logging. Partial = partial logging. No = vendor activity opaque.',
    q2: 'Are vendor activity windows documented (work tickets) and reconcilable against logs?',
    q2_hint: 'Yes = ticket→log reconciliation possible and performed. Partial = tickets exist but not reconciled. No = no documented activity windows.',
  ...
  'DE.CM-09': {
    ...
    q3: 'Are monitoring outputs from EDR/FIM cross-correlated in the SIEM? Show the integration.',
    q3_hint: 'Yes = integrations active with cross-source rules. Partial = data ingested but no correlation. No = sources isolated.',
  ...
  'DE.AE-02': {
    ...
    q1_hint: 'Yes = comprehensive triage runbook. Partial = informal procedures. No = no runbook.',
  ...
  'RS.AN-06': {
    ...
    q2: 'Are investigation records protected from modification (write-once or audit-logged storage, restricted edit rights)?',
    q2_hint: 'Yes = tool config shows audit logs and restricted edits. Partial = some controls. No = records freely editable.',
  ...
  'RS.CO-02': {
    ...
    q3: 'Are notification templates pre-prepared and legal-reviewed so the right message goes out fast under pressure? Sample three.',
    q3_hint: 'Yes = library of legal-reviewed templates per audience. Partial = some templates exist. No = ad-hoc messaging.',
  ...
  'RC.RP-05': {
    ...
    q1_hint: 'Yes = comprehensive checklist with all three check categories. Partial = checklist exists but thin. No = ad-hoc verification.',
  ...
  'RC.RP-06': {
    ...
    q3: 'Are open action items from closed incidents tracked to completion (so closure isn\'t paper-only)? Show the tracker.',
    q3_hint: 'Yes = action items tracked with closure verification. Partial = items tracked but slow closure. No = action items die at closure.',
