...
export const CONTROL_QUESTIONS: Record<string, ControlQuestionnaire> = {
  ...
  'GV.OC-03': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For each major regulation on the register, is current independent compliance evidence on file (a current audit report, third-party attestation, or completed self-assessment) — distinct from the policy-to-control mapping?',
        hint: 'Yes = every applicable regulation has independent evidence on file within its renewal cycle (annual for most, multi-year for some). Partial = some regulations have evidence; others rely on policy text alone. No = compliance is asserted in policy without any independent evidence to back it up.',
      },
    ],
  ...
  'GV.OV-03': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For each KPI, is the data source documented (named tool, named report, named owner) and is at least half of the dashboard sourced from automated tooling rather than manual collection?',
        hint: 'Yes = every KPI cites its source AND ≥50% are pulled from automated tools (EDR console, MDM, IdP reports). Partial = sources documented but most KPIs are manually compiled by one person. No = numbers are presented without sourcing — a board member couldn\'t audit the underlying data.',
      },
    ],
  ...
  'GV.SC-04': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Does the tiering actually drive different operational handling — i.e., a Tier 1 vendor gets more frequent reassessment, stricter contract clauses, or tighter monitoring than a Tier 4 vendor?',
        hint: 'Yes = at least three concrete operational differences by tier (reassessment cadence, contract template, monitoring scope, etc.) are documented and applied. Partial = differentiation exists but is inconsistent across tiers. No = vendors get the same treatment regardless of tier; the labels are cosmetic.',
      },
    ],
  ...
  'GV.SC-06': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For Tier-1 vendors onboarded in the past 12 months, is there a documented 30/60/90-day post-onboarding validation that confirms the security commitments are actually being met in practice?',
        hint: 'Yes = every Tier-1 vendor has a scheduled post-onboarding check within the first 90 days with findings recorded. Partial = validation happens but only informally or on some vendors. No = onboarding ends at contract signature; nothing confirms the commitments were honored.',
      },
    ],
  ...
  'GV.SC-07': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Outside of the scheduled reassessment cadence, what specific signals trigger an interim review (vendor breach disclosure, vendor financial distress, sub-processor change, change in data handled) — and was such a trigger acted on in the past 12 months?',
        hint: 'Yes = the trigger list is documented AND at least one interim review was actually opened in the past year. Partial = triggers documented but never exercised. No = reassessment is fixed-cadence-only with no interim mechanism, so a vendor breach between assessments would not reopen the review.',
      },
    ],
  ...
  'GV.SC-08': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For each Tier-1 vendor named in the IR Plan, is the contractual engagement SLA explicit (e.g., MSP on bridge within 2 hours of incident declaration, identity provider responsive in 1 hour, insurer notified per policy)?',
        hint: 'Yes = engagement SLAs are spelled out in the contract or vendor agreement for every Tier-1 vendor in the IR Plan. Partial = some SLAs are contractual; others are only verbal understandings. No = vendor engagement is best-effort with no contractual response-time commitment.',
      },
    ],
  ...
  'ID.AM-02': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Are SaaS subscriptions reconciled against accounts payable so paid-for services without IT awareness are surfaced — and conversely, IT-approved services without an active subscription get flagged for closure?',
        hint: 'Yes = SaaS-vs-AP reconciliation happens at least quarterly AND surfaces both shadow IT (AP-only) AND zombie licenses (IT-only). Partial = one direction of the reconciliation happens but not both. No = no AP cross-check; departments can subscribe to SaaS without IT visibility.',
      },
    ],
  ...
  'ID.AM-04': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Does every entry in the inventory have a named internal owner who confirms the relationship is still active and still scoped correctly at least annually?',
        hint: 'Yes = each entry has a named owner AND every owner attested to their entries within the last 12 months. Partial = owners assigned but attestation isn\'t happening on cadence. No = entries are unowned or attestation is missing.',
      },
    ],
  ...
  'ID.AM-07': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For each designated data type, does the inventory also map the third parties that receive or process it (sub-processors, integrations, vendor handoffs), with the legal basis on file (BAA, DPA, contract clause)?',
        hint: 'Yes = every regulated data type lists named external recipients AND each recipient has a current BAA / DPA / contract clause on file. Partial = some recipients tracked but legal basis is informal. No = external data flow is not mapped; we can\'t answer "who has this data outside our systems".',
      },
    ],
  ...
  'ID.RA-01': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'When a finding can\'t be remediated within SLA, is there a documented exception with named compensating controls, a risk owner, and a hard expiry date — and are open exceptions reviewed at least quarterly?',
        hint: 'Yes = every SLA miss has a documented exception (compensating controls + owner + expiry) AND open exceptions are reviewed at least quarterly with expired ones either renewed deliberately or closed. Partial = exceptions exist but lack expiry dates or aren\'t reviewed. No = SLA misses go unaddressed without formal exception.',
      },
    ],
  ...
  'ID.RA-03': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Is threat modeling performed proactively for material changes (new SaaS, new integration, new data flow, new exposed service) before they go live — not just retrospectively after an incident?',
        hint: 'Yes = a documented threat-modeling step is a required gate in the change process for material changes AND has been exercised on at least one change in the past 12 months. Partial = threat modeling happens informally or only on the largest changes. No = threats are only catalogued reactively after incidents.',
      },
    ],
  ...
  'ID.RA-05': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'When a risk drops in priority, is it formally decommitted with documented rationale (residual accepted, control retired, scope changed) rather than simply forgotten?',
        hint: 'Yes = every down-prioritization in the past 12 months has a documented rationale on file. Partial = some are documented; others go missing. No = de-prioritized risks fall off the list without any audit trail.',
      },
    ],
  ...
  'PR.AA-01': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For internal role changes (movers — employees who change role/team/department), is the prior role\'s access removed within 14 days, or does old access accumulate as people move around the company?',
        hint: 'Yes = role-change handling has the same rigor as terminations — a documented mover workflow, prior-role access removed within 14 days, sampled and verified. Partial = role changes happen but old access lingers; no SLA. No = movers accumulate permissions over time.',
      },
    ],
  ...
  'PR.AA-03': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For accounts with privileged roles (Global Admin, RMM admin, banking, payment processor, identity provider tenant), is the second factor phishing-resistant (hardware security key or platform passkey) — not an authenticator-app code that can be relayed by an adversary-in-the-middle?',
        hint: 'Yes = every privileged role requires a hardware key or passkey AND the IdP refuses to fall back to a phishable factor for those roles. Partial = hardware keys issued but fallback to TOTP is still possible. No = privileged users authenticate with phishable factors only.',
      },
    ],
  ...
  'PR.AA-05': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Are dormant accounts (no sign-in in 90 days) automatically flagged and disabled — and is a periodic report showing what was disabled reviewed by the access owner?',
        hint: 'Yes = automated 90-day-dormant disable AND a report reviewed at least quarterly with disabled-account decisions logged. Partial = dormant accounts are flagged but disable is manual or untracked. No = dormant accounts can sit indefinitely; no detection mechanism.',
      },
    ],
  ...
  'PR.AT-01': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'After every real or simulated incident in the past 12 months, was the training content updated to reflect what was learned (a new phishing pattern, a new social-engineering pretext, a near-miss) — or does training stay frozen between annual refreshes?',
        hint: 'Yes = at least two content updates in the past year are traceable to a specific incident or sim finding. Partial = one update tied to an incident; the rest is generic refresh. No = training content is frozen between annual reviews regardless of real-world signal.',
      },
    ],
  ...
  'PR.DS-01': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For the keys that protect this data, is key management documented (who holds keys, rotation cadence, escrow/recovery path, separation of key holder from data administrator) — and have keys actually been rotated within the past 12 months?',
        hint: 'Yes = key custody is documented, separation-of-duties enforced between key holder and data admin, AND a rotation event happened within the year for keys under the org\'s control. Partial = keys exist but custody/separation/rotation is informal. No = keys are vendor-managed without escrow or visibility, OR key rotation has never occurred.',
      },
    ],
  ...
  'PR.DS-02': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For machine-to-machine integrations (API calls, webhooks, SFTP transfers), is the transport mutually authenticated (mTLS or signed token + TLS) rather than relying on a static API key over plain TLS?',
        hint: 'Yes = at least the integrations carrying Restricted or Confidential data use mTLS or signed-token-plus-TLS, with no static-shared-secret exposures. Partial = some integrations are hardened; others rely on API keys in plain TLS. No = every integration is API-key over TLS with no mutual auth.',
      },
    ],
  ...
  'DE.CM-01': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For high-severity alerts, what\'s the documented analyst first-touch SLA (e.g., 15 minutes), and what % of alerts in the past month met it 24/7 including weekends and overnight?',
        hint: 'Yes = SLA documented per severity AND ≥90% compliance across all hours including overnight. Partial = SLA met during business hours but degrades off-hours. No = no SLA or coverage is business-hours-only with no after-hours arrangement.',
      },
    ],
  ...
  'DE.CM-09': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Has endpoint isolation (the EDR\'s "network-quarantine" capability) been exercised on a real or simulated case in the past 12 months — and is the isolation lever actually wired to the on-call analyst, not just the EDR vendor?',
        hint: 'Yes = isolation capability is documented, accessible to the on-call AND was used (real or simulated) within the past 12 months. Partial = capability exists but only the vendor can pull the trigger, or it has never been exercised. No = no isolation capability or it\'s blocked by tooling/process.',
      },
    ],
  ...
  'DE.AE-06': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For after-hours, weekends, and holidays, is the on-call rotation explicit (named primary, named backup, escalation chain) and has the rotation actually been paged during off-hours in the past 12 months — or is overnight coverage theoretical?',
        hint: 'Yes = documented on-call rotation with named individuals AND at least one off-hours page in the past year that was acknowledged within SLA. Partial = on-call exists but has never been exercised off-hours, OR the named individuals have churned without rotation updates. No = no off-hours coverage; alerts queue until business hours.',
      },
    ],
  ...
  'DE.AE-07': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'When an IOC arrives, does the analyst sweep historical logs to check whether the indicator was already present (retro-hunt) — and is the retro-hunt window aligned with log retention so it can actually cover the relevant period?',
        hint: 'Yes = retro-hunt is a standard step on incoming IOCs AND the SIEM retains enough log history (typically ≥90 days searchable) to make the hunt meaningful. Partial = retro-hunts happen ad-hoc, or retention is too short to be useful. No = IOCs are dropped into prospective detection only; the past never gets re-examined.',
      },
    ],
  ...
  'DE.AE-08': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'When the criteria are applied, do multiple analysts reach the same classification on the same case (inter-rater consistency)? Has a recent QA review tested this?',
        hint: 'Yes = a QA review within 12 months tested at least one case across multiple analysts and the classifications agreed (or disagreements led to criteria refinement). Partial = consistency is assumed but never tested. No = each analyst classifies on gut feel; no consistency check.',
      },
    ],
  ...
  'RS.MA-01': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For every regulation that applies to this organization (state breach-notification law, HIPAA, PCI, contractual SLAs), is the required notification timeline written into the IR Plan with a triggering threshold — and has the timeline been map-tested against a recent or simulated incident?',
        hint: 'Yes = every applicable regulation has its notification timeline named in the plan AND a map-test (real or tabletop) within 12 months confirmed the team can hit it. Partial = some regulations covered; timelines exist but were never tested under pressure. No = the plan doesn\'t enumerate notification timelines per regulation.',
      },
    ],
  ...
  'RS.AN-03': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Does the RCA process operate under a blameless / just-culture framing — where individual human error is examined as a system failure rather than a disciplinary event — and is that framing documented in policy so analysts and witnesses participate honestly?',
        hint: 'Yes = blameless / just-culture framing is explicit in policy AND analysts confirm RCAs feel safe to participate in (test: ask a recent participant). Partial = the framing is implicit; some RCAs feel safe and others have led to performance consequences. No = RCAs are treated as disciplinary; witnesses self-censor.',
      },
    ],
  ...
  'RS.CO-02': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'When customer notification is required, is there an out-of-band channel ready (status page, postal mail vendor, customer-portal banner) — distinct from email — so notification still works if the primary email system is itself compromised?',
        hint: 'Yes = at least one out-of-band channel exists, is operational, and has been validated in the past 12 months. Partial = a backup channel exists but has never been validated. No = customer notification depends entirely on the email system that may be the thing under attack.',
      },
    ],
  ...
  'RS.MI-01': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'After containment is declared on an incident, is there an objective verification step (re-scan, sweep for indicators, monitoring for lateral movement) before the case is moved to eradication — rather than relying on absence-of-alerts as proof?',
        hint: 'Yes = a documented verification check runs after every containment declaration AND the check is distinct from "no new alerts." Partial = verification is informal or only happens on Critical incidents. No = containment is declared on absence of alerts; no proactive verification.',
      },
    ],
  ...
  'RC.RP-02': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'Does the prioritization rubric account for technical dependencies — i.e., system A can\'t be recovered before its database B, and the order respects this — and has the dependency map been validated in the past 12 months?',
        hint: 'Yes = the rubric explicitly captures upstream/downstream dependencies AND a current dependency map was validated within the year. Partial = dependencies are accounted for informally; map is missing or stale. No = recovery order is decided on business priority alone, with technical dependencies discovered during execution.',
      },
    ],
  ...
  'RC.RP-03': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For a ransomware-specific scenario, has a restore-from-clean-snapshot been tested — restoring backups that pre-date a hypothetical compromise window, validating that the malware does not re-emerge — distinct from an operational-restore drill?',
        hint: 'Yes = a ransomware-recovery rehearsal has been performed within 12 months, restoring from a snapshot at least 7 days old AND validating that no compromise artifacts re-appear. Partial = operational restores are tested but ransomware-specific scenarios have not been rehearsed. No = no ransomware rehearsal; the team has never restored from a pre-compromise snapshot.',
      },
    ],
  ...
  'RC.CO-04': {
    items: [
      ...
      {
        id: 'q4',
        prompt: 'For a major public incident, is there a designated executive spokesperson (CEO or named delegate) trained or rehearsed in media handling — not an IT lead defaulting into the role under pressure?',
        hint: 'Yes = named spokesperson AND documented media-handling rehearsal or training within 24 months. Partial = spokesperson named but untrained. No = no designated spokesperson; whoever is available speaks for the company.',
      },
    ],
